...
//            ClassLoader rootLoader = DefaultGroovyMethods.getRootLoader(loader);
            ...
            // try using the classloader from the current static class
            if (found == null) {
                try {
                    ClassLoader classLoader = Class.forName("org.apache.groovy.json.internal.FastStringUtils")
                            .getClassLoader();
                    serviceLoader = ServiceLoader.load(FastStringServiceFactory.class, classLoader);
                    for (FastStringServiceFactory factory : serviceLoader) {
                        FastStringService service = factory.getService();
                        if (service != null) {
                            found = service;
